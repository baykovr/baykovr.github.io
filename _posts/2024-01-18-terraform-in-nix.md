...
From this environment, we may readily generate an OCI image for use in CI/CD systems without the need to author a Dockerfile or additional scripts.

Tersely, we use nix to provide the combined functionality of to python's virtualenv and docker to terraform.
...
## Approach
We define a `default.nix` file for our terraform environment which includes the terraform binary itself, set of several providers addons and two targets of `terraformShell` and `terraformImage`.

Thus `default.nix` function of a build script such as `make(1)`.  
...
### Usage
Creating a local shell environment is thus. 
```bash
...
Creating an image from this environment is thus.
```bash
...
The resulting image is optimial in size, only the required dependencies are baked in akin to [google/distroless](https://github.com/GoogleContainerTools/distroless). 
...
Next, we define a `flake.nix` in order to pin nix and its packages.

This example shows handling of terraform's BSL license as well as flake-utils, which allows us to build on other architectures such as arm.
...
A lockfile is written, further locking the environment.
